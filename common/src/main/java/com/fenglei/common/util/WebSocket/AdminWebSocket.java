 ...
 */
@Component
@ServerEndpoint("/websocketAdmin/{userId}")
public class AdminWebSocket {

    private Session session;

    private static final CopyOnWriteArraySet<AdminWebSocket> WEB_SOCKETS = new CopyOnWriteArraySet<>();
    ...
    public void sendMessage(SysMsg msg) {
        Session session = SESSION_POOL.get("admin" + msg.getUserId());
        if (session != null) {
            try {
