...
public class DictItemController {
        ...
        switch (queryModeEnum) {
            ...
            case LIST:
                List<SysDictItem> list = iSysDictItemService.list(new LambdaQueryWrapper<SysDictItem>()
                        ...
                        .select(SysDictItem::getName, SysDictItem::getValue, SysDictItem::getRemark)
    ...
    public Result add(@RequestBody SysDictItem dictItem) {
        ...
        if (list != null && list.size() > 0) {
            throw new BizException("所选字典类别下已存在该字典值，无法新增");
        }
    ...
    public Result update(
        ...
        List<SysDictItem> list = iSysDictItemService.list(
                new LambdaQueryWrapper<SysDictItem>()
                        .eq(SysDictItem::getDictCode, dictItem.getDictCode())
                        .eq(SysDictItem::getValue, dictItem.getValue())
                        .ne(SysDictItem::getId, dictItem.getId())
        );
        if (list != null && list.size() > 0) {
            throw new BizException("所选字典类别下已存在该字典值，无法新增");
        }
    ...
    public Result getByCode(@RequestParam("code") String code) {
        List<SysDictItem> list = iSysDictItemService.list(new LambdaQueryWrapper<SysDictItem>()
                .eq(StrUtil.isNotBlank(code), SysDictItem::getDictCode, code)
                .select(SysDictItem::getName, SysDictItem::getValue)
                .orderByAsc(SysDictItem::getSort));
        return Result.success(list);
    }

    @ApiOperation(value = "根据 dictItem 查询字典数据")
    @GetMapping("/getByDictItem")
    public Result getByDictItem(@RequestParam("dictItem") SysDictItem dictItem) {
        SysDictItem item = null;
        List<SysDictItem> list = iSysDictItemService.list(new LambdaQueryWrapper<SysDictItem>()
                .eq(StrUtil.isNotBlank(dictItem.getDictCode()), SysDictItem::getDictCode, dictItem.getDictCode())
                .eq(StrUtil.isNotBlank(dictItem.getName()), SysDictItem::getName, dictItem.getName())
                .eq(StrUtil.isNotBlank(dictItem.getValue()), SysDictItem::getValue, dictItem.getValue())
                .orderByAsc(SysDictItem::getSort));
        if(list.size() > 0) {
            item = list.get(0);
        }
    ...
    @GetMapping("/listForList")
    public Result listForList(
            String queryMode,
            Integer page,
            Integer limit,
            String name,
